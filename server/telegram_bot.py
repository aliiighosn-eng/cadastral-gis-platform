...
class GazpromTelegramBot:
    ...
    async def start_parse_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    async def handle_cadastral_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            if property_data:
                ...
                await update.message.reply_text(
                    f"❌ Объект с кадастровым номером {cadastral_number} не найден.\n"
    ...
    async def create_geojson_file(self, property_data: Dict) -> str:
        ...
        filename = f"/tmp/property_{property_data.get('cadastral_number', 'unknown').replace(':', '_')}.geojson"
    ...
    def format_property_info(property_data: Dict) -> str:
        ...
        message = (
            ...
            f"🔢 Кадастровый номер: {property_data.get('cadastral_number', 'N/A')}\n"
            f"📍 Адрес: {property_data.get('address', 'N/A')}\n"
            f"📐 Площадь: {property_data.get('area', 'N/A')} м²\n"
            f"🏗️ Назначение: {property_data.get('land_use', 'N/A')}\n"
        )
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        help_text = (
            ...
            "Для получения информации о кадастровом объекте используйте команду /start_parse"
        )
    ...
    def setup_handlers(self) -> None:
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                SELECTING_TYPE: [
                    MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_type_selection)
                ],
                ENTERING_CADASTRAL: [
